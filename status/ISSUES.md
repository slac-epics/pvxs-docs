...
| Fixed | #                                                       | Issue                                                                             | Author                       | Age       | Fix / PR                                                                                                                                    | Details                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| :---: | ------------------------------------------------------- | --------------------------------------------------------------------------------- | ---------------------------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ✅ | [#19](https://github.com/slac-epics/pvxs-cms/issues/19) | Scriptability                                                                     | mdavidsaver                  | 18d       | **PR [#21](https://github.com/slac-epics/pvxs-cms/pull/21) · needs-review** | Has a CLI-direction design decision (semantic conflict with merged #29). Wants `pvacms --init`/`--run`, cert-ID-only-to-stdout. PR #21 implements `--run-while`, `pvxcert --status-pv`, `pvxcert <status:pv>`. Rebase onto post-#29 `main` deliberately NOT done: #21 makes the positional arg a full status PV and drops the `<issuer>:<serial>` form, which is mutually exclusive with #29's `<issuer>:<serial>` + `--cert-pv-prefix`. Left for a CLI-direction decision.                                                                     |
...
| ✅ | [#26](https://github.com/slac-epics/pvxs-cms/issues/26) | Use POSIX time everywhere - eliminate EPICS_EPOCH (CMS)                           | george-mcintyre              | 7d        | **PR [#40](https://github.com/slac-epics/pvxs-cms/pull/40) · needs-review** | Changes responded to; awaiting re-review. NT `timeStamp.secondsPastEpoch` now carries POSIX seconds (per epics-base pvData TimeStamp), custom `renew_by` field made raw POSIX `time_t`. Fixes the `renew_by` double-subtraction (~1986) that is the date symptom (root cause B) of the now-closed umbrella #7. mdavidsaver's review (3 comments: throw on time-source failure, drop dead code, fix test cast) addressed; re-review requested. DB already POSIX. pvxs-tls #20 (lib side) still open.                                                                                                                                    |
...
| · | [#20](https://github.com/slac-epics/pvxs-cms/issues/20) | authn* CertCreationRequest are not X509_REQ                                       | mdavidsaver                  | 18d       | not-started                                                                                                                                                             | The RPC request from `authnstd`→`pvacms` is unsigned data possibly sent in the clear; should use openssl `X509_REQ` (signed CSR) instead.                                                                                                                                                                                                                                                                                                                         |
| · | [#23](https://github.com/slac-epics/pvxs-cms/issues/23) | Misc. issues                                                                      | mdavidsaver                  | 14d       | not-started                                                                                                                                                             | Grab-bag: default cert type should be `ioc` not `client`; keychain extension check should warn not error; CMS tool under `tls_disabled` blocks a TLS-only environment.                                                                                                                                                                                                                                                                                            |
...
|---|----|--------|-----|--------|------------|
| [#18](https://github.com/slac-epics/pvxs-tls/pull/18) | fix(client): copy monitor callback before invoking to survive re-entrant cancel | george-mcintyre | 33d (~4w) | needs-review | Monitor-callback copy fix (on `dev`); also unblocks pvxs-cms #16. (The broader ASan callback-lifetime audit is now handled upstream: epics-base/pvxs#188, commits `0b3fcca1` + `39cc6fa2`.) |
| [#17](https://github.com/slac-epics/pvxs-tls/pull/17) | feat: tls-only transport mode (`EPICS_PVAS_TLS_OPTIONS=no_tcp`) | george-mcintyre | 35d (~5w) | changes-requested (not yet responded to) | New feature: allow a TLS-only server (no TCP fallback). Relates to pvxs-cms [#23](https://github.com/slac-epics/pvxs-cms/issues/23) (CMS tools blocked in TLS-only env). |
| [#13](https://github.com/slac-epics/pvxs-tls/pull/13) | fix: demote benign loopback echo of own UDP search to Debug | george-mcintyre | 56d (~8w) | needs-review (changes responded to) | Log-noise fix: own broadcast search echoed back is logged too loudly. |
| [#7](https://github.com/slac-epics/pvxs-tls/pull/7) | fix: tear down live TLS connections when local cert goes BAD | george-mcintyre | 71d (~2mo) | needs-review | Security-relevant: drop established TLS connections when the local cert transitions to BAD. Relates to fail-secure theme ([#26](https://github.com/slac-epics/pvxs-tls/issues/26)). |
